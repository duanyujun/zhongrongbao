...
- (IBAction)btnDownHandler:(id)sender
{
    ...
    [self setLayout];
    [delegate refreshEnterpriseBriefIntroduction];
}
...
- (void)setLayout
{
    ...
    btn.center = CGPointMake(btn.center.x, CGRectGetMaxY(contentLabel.frame)+15.0f);
    imgView.center = CGPointMake(imgView.center.x, btn.center.y);
    CGRect ownFrame = self.frame;
    ownFrame.size.height = CGRectGetMaxY(btn.frame) + 20;
    self.frame = ownFrame;
